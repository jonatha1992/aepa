...
import { agregarDocSub, agregarSubcoleccionDoc, uploadFilesConte, getDocumentCount } from "../firebase"; // Importa la nueva función
...
export default function AltaModulos({ cursoId }) {
    ...
    const handleAgregarModulo = async () => {
        ...
        try {
            ...
            const moduloRef = await agregarDocSub(moduloData, `cursos/${cursoId}/Modulos`);
            ...
            for (const item of items) {
                ...
                await agregarSubcoleccionDoc(itemData, `cursos/${cursoId}/Modulos/${moduloRef.id}/items`);
            }
            ...
            alert("Hubo un error al agregar el módulo. Por favor, intenta nuevamente.");
        } finally {
            setLoading(false);
        }
    };
                    ...
                    {items.map((item, index) => (
                        ...
                        >
                            <TextField
                                ...
                                helperText={errors.items[index]?.titulo && "Este campo es obligatorio"}
                            />
                            <FormControl fullWidth margin="normal" error={errors.items[index]?.tipo}>
                                <InputLabel>Tipo</InputLabel>
                                <Select value={item.tipo} onChange={(e) => handleChangeItem(e, index, "tipo")} label="Tipo">
                                    <MenuItem value="link">Link</MenuItem>
                                    <MenuItem value="pdf">PDF</MenuItem>
                                    <MenuItem value="info">Info</MenuItem>
                                </Select>
                                {errors.items[index]?.tipo && <p style={{ color: "red" }}>Este campo es obligatorio</p>}
                            </FormControl>
                            {item.tipo === "link" ? (
                                <TextField
                                    label="URL"
                                    ...
                                    helperText={errors.items[index]?.file && "Este campo es obligatorio"}
                                />
                            ) : (
                                <>
                                    <Button variant="contained" component="label" fullWidth sx={{ marginTop: 1 }}>
                                        Seleccionar Archivo
                                        <input type="file" hidden onChange={(e) => handleChangeItem(e, index, "file")} />
                                    </Button>
                                    {item.file && <p>Archivo seleccionado: {item.file.name}</p>}
                                </>
                            )}
                            {errors.items[index]?.file && <p style={{ color: "red" }}>Este campo es obligatorio</p>}
