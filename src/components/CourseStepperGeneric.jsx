...
export default function CourseStepperGeneric({ cursoId, onCursoActualizado }) {
    ...
    for (let field of fields) {
      if (Array.isArray(courseData[field])) {
        ...
        if (!courseData[field].some((item) => item.trim() !== "")) {
          toast.error(
            `Por favor añade al menos un ${field} en ${steps[activeStep]}`
          );
          return false;
        }
      } else {
        ...
        if (
          ...
          if (!courseData[field] || courseData[field].trim() === "") {
            toast.error(
              `Por favor completa el campo ${field} en ${steps[activeStep]}`
            );
            return false;
          }
        }
        ...
        if (["price", "workload"].includes(field)) {
          if (isNaN(courseData[field]) || courseData[field] <= 0) {
            toast.error(
              `Por favor ingresa un valor válido para ${field} en ${steps[activeStep]}`
            );
            return false;
          }
        }
  ...
  const handleChange = (event, field, index = null) => {
    const value =
      field === "habilitado" ? event.target.checked : event.target.value;
    if (index !== null) {
        ...
        <Box sx={{ display: "flex", flexDirection: "column", pt: 2 }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "row", pt: 2 }}>
            ...
            <FormControlLabel
              control={
                <Switch
                  checked={courseData.habilitado}
                  onChange={(e) => handleChange(e, "habilitado")}
                  name="habilitado"
                />
              }
              label="Habilitado"
